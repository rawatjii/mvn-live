...
const metaFields = [
  ...
  { name: "links", label: "Link Url", type: "text", col: 12, isLeft: true },
...
const columns = [
  ...
  { key: "links", label: "Link Url" },
  { key: "image", label: "Image", type: "file" },
];

export default function OtherVerticals(){
  const [editModalData, setEditModalData] = useState(null);

  const aboutsApi = generateApi("verticals");
  ...
  return (
    <CustomSection customClass="">
      ...
      <LeftArea>
        <MicroBox>
          <CustomTitle title="Other Verticals Form" />
          <CustomForm
            isBanner={false}
      ...
      <RightArea>
        <MicroBox>
          <CustomTitle title="Other Verticals Table" />
          <CustomTable
            columns={columns}
            data={paginatedData}
            onEdit={handleEdit} 
