...
const Watermark = ({ className, type, isMvnLogo, isAthensLogo, customClass }) => {
  ...
  useEffect(() => {
    if (
      location.pathname === "/mvn-athens-faridabad" ||
      location.pathname === "/mvn-athens-gurgaon-phase-2" ||
      location.pathname === "/mvn-athens-gurgaon-phase-1"
    ) {
    ...
    default:
      return (
        <div className={`WaterMarkContainer ${customClass}`}>
          ...
          {isMvnLogo ? (
            <div
              className={`Watermark_logo ${
                isAeroGurgaon ? "aeroGurgaon_logo" : null
              }`}
            >
              <img
                src={`${CONFIG.API_URL}assets/mvn_mall.webp`}
                alt="mvn logo image"
              />
            </div>
          ) : isAthensLogo ? (
            <div
              className={`Watermark_logo ${
                isAeroGurgaon ? "athens_logo" : null
              }`}
            >
              <img
                src={`${CONFIG.API_URL}assets/athens_logo.webp`}
                alt="mvn logo image"
              />
            </div>
          ) : (
            ...
            >
              <img src={logoUrl} alt="mvn logo image" />
            </div>
          )}
        </div>
      );
